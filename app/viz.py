...
def render_training(analysis, labels, show_overlay=True, chosen=None,
    ...
    for pid, (cls, from_user) in labels.items():
        ...
        col = PATTERN_COLORS.get(cls) or (GREEN if cls == "undercooled" else None)
        if col is None:
            continue
        lut[pid] = col
        fill[pid] = True
...
def render(analysis, show_measurements=True, show_classification=False,
    ...
    # (excluded/dim particles are NOT tinted in the class/pattern overlay — a
    # slate tint there reads as "stripe" and confuses. They're only greyed in
    # Measure mode below, where the grey clearly means "not measured".)
