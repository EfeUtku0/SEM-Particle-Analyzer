...
pattern_train.py). Crops are raw (unblurred) 128px patches, resized and
robust-normalized at train/inference time by patterncrop.featurize_batch —
...
def pattern_probs(gray, masks, regions, nm_per_px):
    ...
    for rp in regions:
        im, m = patterncrop.raw_crop(g, masks, rp.label, rp.bbox)
        if im is not None:
    ...
    if not ids:
        return {}
    X = patterncrop.featurize_batch(np.array(imgs), np.array(sils), np.array(dnm))
    Xt = torch.tensor(X)
    net = _get_net()
    out = []
    with torch.no_grad():
