...
def _retrained_path():
    """A retrained gate lands in the data folder and wins over the bundled one.

    Same arrangement patternnet has had since it became trainable in-app, and
    for the same reason: a frozen app bundle is read-only. It matters more here
    than it looks — until 2026-08-04 this module read the bundled file and
    nothing else, so the solid/liquid gate was the one model in the pipeline
    that could not be improved without rebuilding the whole app. That is why a
    second microscope's photos could break it and stay broken however much the
    user labelled (the in-app "Train model" only ever touched patternnet).
    """
    from paths import data_dir
    return os.path.join(data_dir(), "solidnet.pt")
...
def reload():
    """Drop the cached network so the next call picks up a retrained file."""
    global _net
    _net = None
